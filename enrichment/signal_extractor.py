...
def _build_signal_checklist(signals: list[dict]) -> str:
    ...
    for s in signals:
        if s.get("source_type") == "static_lookup":
            raise ValueError(
                f"Signal {s['signal_id']!r} has source_type=static_lookup, "
                "which is not implemented. A static_lookup signal cannot be evaluated "
                "against website text — remove source_type from the ICP profile or "
                "implement a resolver before running."
            )
...
def _validate_and_clean_signals(raw_signals: list[dict],
    ...
    for sig in raw_signals:
        ...
        validated_map[signal_id] = {
            ...
            "required_for_contender": bool(icp_by_id[signal_id].get("required_for_contender", False)),
            "cap_tier": icp_by_id[signal_id].get("cap_tier", ""),
            "floor_tier": icp_by_id[signal_id].get("floor_tier", ""),
            "exclude_if_yes": bool(icp_by_id[signal_id].get("exclude_if_yes", False)),
            "inhibited_by": icp_by_id[signal_id].get("inhibited_by"),
