...
**target_tier:** `"Bullseye"`, `"Needs Verification"`, `"Contender"`, `"Manual Review"`, or `"Excluded"` only (call-tier rank order, highest first: Bullseye > Needs Verification > Contender). `"Needs Verification"` is a CLEAR record that scored as a candidate but has an unconfirmed `verification_required` / `required_for_bullseye` (must-have) signal — call to confirm before shipping. `"Contender"` is a solid-fit CLEAR record a notch below Bullseye. `"Manual Review"` is a CLEAR record held out of the call queue and client exports until an operator acts; it is *not* a fit verdict. A record routes there when **any** of these hold: zero confirmed evidence (no `"yes"` and nothing inferred); `bullseye_score` below the low-score floor with no `floor_tier` guarantee; `source_confidence` of `"limited"` or `"failed"` (blocked / thin crawl); or an unconfirmed `required_for_contender` qualifier. `"Excluded"` appears if and only if `exclusion_status == "EXCLUDED"`. (The Contender tier was formerly named "Watchlist".)
