...
2. **`not_found_reason` field.** Four situations produce `signal_state: not_found` with different rep implications: (a) crawled successfully, service absent; (b) site couldn't be crawled; (c) LLM claimed "yes" but the evidence gate downgraded it (missing quote or source URL); (d) LLM claimed "yes" but the evidence was a physician bio, blog/education page, referral-out, testimonial, or historical mention rather than an offered service (attribution guard). The `not_found_reason` field (`""` / `"no_context"` / `"evidence_gate"` / `"attribution_gate"`) is in the schema and surfaced in the UI but the distinction is not yet exposed in client exports.
